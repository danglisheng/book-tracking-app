...
class BooksApp extends React.Component {
  ...
  displayDropdownItem(prevShelf,curShelf,dropDown) {
    ...
    if(prevShelf){
      dropDown.querySelector(`option[value=${prevShelf}]`).style.display="block";
    }
  ...
  changeBookType(book,curShelf,dropDown) {
    BooksAPI.update(book,curShelf).then((result)=>{
      console.log(result);
      this.displayDropdownItem(book.shelf,curShelf,dropDown);
      this.changeLocalBooksArr(book,curShelf);
    }).catch((error)=>{
      console.log(error);
      alert("An error happens,the changes can't be saved on the server:"+error.toString());
    });
  }
